...
def prod_list_rec_full(lst):
    ...
    if not isinstance(head, Number):
        print("Non-numeric element encountered:", head)
        return  # returns None
    tail_prod = prod_list_rec_full(tail)
    if tail_prod is None:
        return
    return head * tail_prod
